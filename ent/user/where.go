...
// HasLikedArticles applies the HasEdge predicate on the "liked_articles" edge.
...
// HasLikedArticlesWith applies the HasEdge predicate on the "liked_articles" edge with a given conditions (other predicates).
func HasLikedArticlesWith(preds ...predicate.Article) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newLikedArticlesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
