...
func (fq *FavoriteQuery) QueryArticle() *ArticleQuery {
...
func (fq *FavoriteQuery) Clone() *FavoriteQuery {
	...
	return &FavoriteQuery{
		...
		withArticle: fq.withArticle.Clone(),
...
func (fq *FavoriteQuery) WithArticle(opts ...func(*ArticleQuery)) *FavoriteQuery {
	query := (&ArticleClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withArticle = query
	return fq
}
...
//		GroupBy(favorite.FieldFavoritedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Favorite.Query().
//		Select(favorite.FieldFavoritedAt).
//		Scan(ctx, &v)
...
func (fq *FavoriteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Favorite, error) {
	...
	if query := fq.withArticle; query != nil {
		if err := fq.loadArticle(ctx, query, nodes, nil,
			func(n *Favorite, e *Article) { n.Edges.Article = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FavoriteQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*Favorite, init func(*Favorite), assign func(*Favorite, *Article)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Favorite)
	for i := range nodes {
...
func (fq *FavoriteQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := fq.ctx.Fields; len(fields) > 0 {
		...
		if fq.withArticle != nil {
			_spec.Node.AddColumnOnce(favorite.FieldArticleID)
		}
	}
