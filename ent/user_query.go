...
// QueryLikedArticles chains the current query on the "liked_articles" edge.
...
// the "liked_articles" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLikedArticles; query != nil {
		if err := uq.loadLikedArticles(ctx, query, nodes,
			func(n *User) { n.Edges.LikedArticles = []*Article{} },
			func(n *User, e *Article) { n.Edges.LikedArticles = append(n.Edges.LikedArticles, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadLikedArticles(ctx context.Context, query *ArticleQuery, nodes []*User, init func(*User), assign func(*User, *Article)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "liked_articles" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
